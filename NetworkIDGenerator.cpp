...
SystemAddress NetworkIDGenerator::externalSystemAddress=UNASSIGNED_SYSTEM_ADDRESS;
...
//-------------------------------------------------------------------------------------
void NetworkIDGenerator::SetExternalSystemAddress(SystemAddress systemAddress)
{
	assert(systemAddress!=UNASSIGNED_SYSTEM_ADDRESS);
	externalSystemAddress=systemAddress;
}
//-------------------------------------------------------------------------------------
SystemAddress NetworkIDGenerator::GetExternalSystemAddress(void)
{
...
void NetworkIDGenerator::GenerateID(void)
{
	...
	do
	{
		networkID.localSystemAddress=staticItemID++;
		if (NetworkID::peerToPeerMode)
		{
			 // If this assert hits you forgot to call SetExternalSystemAddress
			assert(externalSystemAddress!=UNASSIGNED_SYSTEM_ADDRESS);
			networkID.systemAddress=externalSystemAddress;
		}
